...
function! vimfluency#pinpoints#move_to_line_edges_non_white_space#lesson() abort
  ...
  return [
    ...
    \  'prompt': [
    \    'Two non-blank line-edge motions:',
    \    '',
    \    '    ^   →  first NON-BLANK char (skips leading whitespace)',
    \    '    g_  →  last NON-BLANK char (skips trailing whitespace)',
    \    '',
    \    'Trailing whitespace shows as · so the difference vs $ is visible.',
    \    '',
    \    'Press <Space> to continue.']},
    \ {'kind': 'try', 'lines': buf, 'start': [1, 10], 'target': [1, 5],
    \  'prompt': 'Press ^ — lands on the first non-blank (the i of "if").'},
    \ {'kind': 'try', 'lines': buf, 'start': [1, 10], 'target': [1, 19],
    \  'prompt': 'Press g_ — lands on the last non-blank (the n of "return").'},
...
function! vimfluency#pinpoints#move_to_line_edges_non_white_space#generate() abort
