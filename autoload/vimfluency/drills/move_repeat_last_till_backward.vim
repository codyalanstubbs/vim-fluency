...
function! vimfluency#drills#move_repeat_last_till_backward#lesson() abort
  ...
  return [
    ...
    \  'waypoints': [[1, 11]],
    \  'prompt': 'Both s''s are behind you. Press Ts (lands one after the nearer s, col 11 = 1), then ; (skips back to one after the first s, col 5 = 2).'},
    \ {'kind': 'try', 'lines': buf1, 'start': [1, 6], 'target': [1, 9],
    \  'waypoints': [[1, 5]],
    \  'prompt': 'You sit between the s''s. Press Ts (lands one after the first, col 5 = 1), then , (reverses like t — one BEFORE the second s, col 9 = 2).'},
    \ {'kind': 'try', 'lines': buf1, 'start': [1, 12], 'target': [1, 5],
    \  'waypoints': [[1, 11]],
