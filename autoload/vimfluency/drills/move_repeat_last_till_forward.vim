...
function! vimfluency#drills#move_repeat_last_till_forward#lesson() abort
  ...
  return [
    ...
    \  'prompt': [
    \    'After t (one cell before the match), ; and , repeat it — no retyping:',
    \    '',
    \    '    ;   →   repeats it forward again (one before the next match)',
    \    '    ,   →   reverses, like T (one after the previous match)',
    \    '',
    \    'Same read as the find version, but the cursor lands beside the',
    \    'match, not on it.',
    \    '',
    \    'Press <Space> to continue.']},
    \ {'kind': 'try', 'lines': buf1, 'start': [1, 2], 'target': [1, 9],
    \  'waypoints': [[1, 3]],
    \  'prompt': 'Both s''s are ahead. Press ts (lands one before the first s, 1), then ; (skips to one before the second s, 2).'},
    \ {'kind': 'try', 'lines': buf1, 'start': [1, 8], 'target': [1, 5],
    \  'waypoints': [[1, 9]],
    \  'prompt': 'You sit between the s''s. Press ts (lands one before the second, 1), then , (reverses like T — one AFTER the first s, 2).'},
    \ {'kind': 'try', 'lines': buf1, 'start': [1, 2], 'target': [1, 9],
    \  'waypoints': [[1, 3]],
