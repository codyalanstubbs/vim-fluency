...
" move_repeat_last_till_forward: t-only ; / , with constant-shape
" geometry. Same invariants as the find sibling, but the waypoint and
" target sit BESIDE the occurrences (off-by-one), reflecting t's
" lands-before semantics and ,'s T-like reverse:
"   ; : t -> p-1 (waypoint), ; -> q-1 (target)
"   , : t -> q-1 (waypoint), , -> p+1 (target)
function! s:test_move_repeat_last_till_forward() abort
  let GenFn = function('vimfluency#drills#move_repeat_last_till_forward#generate')
  let valid = [';', ',']
  let seen = {}
  let prefix = 'move_repeat_last_till_forward: '
  for i in range(s:N)
    let item = GenFn()
    ...
    " Locate the two occurrences (the search char appears exactly twice).
    " The search char is whatever sits adjacent to the landings: for the
    " off-by-one t landing, the actual matches are derived below.
    let p = 0 | let q = 0
    if item.expected_motion ==# ';'
      " waypoint = p-1, target = q-1  ->  p = wp+1, q = tc+1
      let p = wp + 1
      let q = tc + 1
      call AssertEq(sc, p - 2, prefix . '; cursor is p-2')
    else
      " waypoint = q-1, target = p+1  ->  q = wp+1, p = tc-1
      let q = wp + 1
      let p = tc - 1
      call AssertEq(sc, p + 4, prefix . ', cursor is p+4')
    endif
    ...
    call AssertEq(occ, [p, q], prefix . 'search char appears exactly at p and q')

    " Targets interior (defeat 0/^/$) and cursor not on a match.
    call Assert(tc >= 4 && tc <= llen - 1, prefix . 'target interior')
    call Assert(line[sc - 1] !=# search, prefix . 'cursor not on search char')
  endfor
  for m in valid
    call Assert(get(seen, m, 0) == 1, prefix . m . ' appeared in samples')
...
" move_repeat_last_till_backward: T-only ; / , — the backward mirror.
" Off-by-one landings on the FAR side of the backward match:
"   ; : T -> q+1 (waypoint), ; -> p+1 (target)
"   , : T -> p+1 (waypoint), , -> q-1 (target)
function! s:test_move_repeat_last_till_backward() abort
  let GenFn = function('vimfluency#drills#move_repeat_last_till_backward#generate')
  let valid = [';', ',']
  let seen = {}
  let prefix = 'move_repeat_last_till_backward: '
  for i in range(s:N)
    let item = GenFn()
    ...
    if item.expected_motion ==# ';'
      ...
      let q = wp - 1
      let p = tc - 1
      call AssertEq(sc, q + 2, prefix . '; cursor is q+2')
    else
      " waypoint = p+1, target = q-1  ->  p = wp-1, q = tc+1
      let p = wp - 1
      let q = tc + 1
...
" move_repeat_last_till_forward_backward: t/T mix. Delegates to the two
" single-direction generators (each independently shape-verified), so
" this test confirms the delegation: valid {; ,} items, and that BOTH
" setups appear. The first till always moves the cursor exactly one
" cell to the waypoint, so |cursor - waypoint| == 1 and the sign
" identifies the direction (cursor left of waypoint = forward t;
" cursor right = backward T).
function! s:test_move_repeat_last_till_forward_backward() abort
  let GenFn = function('vimfluency#drills#move_repeat_last_till_forward_backward#generate')
  let valid = [';', ',']
  let seen = {}
  let saw_t = 0 | let saw_T = 0
  let prefix = 'move_repeat_last_till_forward_backward: '
  for i in range(s:N)
    let item = GenFn()
    ...
    let sc = item.start[1]
    let wp = item.waypoints[0][1]
    call AssertEq(abs(sc - wp), 1, prefix . 'cursor one cell from the waypoint')
    if sc < wp | let saw_t = 1 | else | let saw_T = 1 | endif
    let seen[item.expected_motion] = 1
  endfor
  for m in valid
    call Assert(get(seen, m, 0) == 1, prefix . m . ' appeared in samples')
  endfor
  call Assert(saw_t, prefix . 'forward-till (t) setup appeared')
...
" move_repeat_last_find_vs_till_backward: F/T mix, backward mirror of
" the f/t test. find (F) steps 2-3 cells to the waypoint (lands on),
" till (T) steps exactly 1 (lands after), so |cursor - waypoint| == 1
" is till and > 1 is find. Confirms both families appear.
function! s:test_move_repeat_last_find_vs_till_backward() abort
  let GenFn = function('vimfluency#drills#move_repeat_last_find_vs_till_backward#generate')
  let valid = [';', ',']
  let seen = {}
  let saw_find = 0 | let saw_till = 0
  let prefix = 'move_repeat_last_find_vs_till_backward: '
  for i in range(s:N)
    let item = GenFn()
...
" (delimiter and angle/tag families). Each item: single line, cursor
" strictly interior to the red range, optimal 1, cursor lands at the
" deletion start; the real motion reproduces target_lines/target; and no
" <=3-keystroke alternative in `cheats` reproduces both (the cheat gate).
function! s:assert_inner_object_drill(id, valid, cheats) abort
  let GenFn = function('vimfluency#drills#' . a:id . '#generate')
  let seen = {}
  for i in range(s:N)
    let item = GenFn()
    ...
    call AssertIn(item.expected_motion, a:valid,
      \ a:id . ': expected_motion in ' . string(a:valid))
    call AssertEq(item.optimal_motions, 1, a:id . ': optimal_motions == 1')
    call AssertEq(len(item.lines), 1, a:id . ': single-line buffer')

    let dr = item.deletion_range[0]
    call Assert(item.start[1] > dr[1] && item.start[1] < dr[1] + dr[2],
      \ a:id . ': cursor starts strictly inside the highlight')
    call AssertEq(item.target, [1, dr[1]],
      \ a:id . ': cursor lands at the deletion start col')
    ...
    execute 'normal! ' . item.expected_motion
    call AssertEq(getline(1, '$'), item.target_lines,
      \ a:id . '/' . item.expected_motion . ': buffer matches target_lines')
    call AssertEq([line('.'), col('.')], item.target,
      \ a:id . '/' . item.expected_motion . ': cursor matches target')
    bwipeout!

    " Cheat gate: no alternative in `cheats` may reproduce both the
    " target buffer and cursor (diw/daw are the dangerous word-object
    " ties; dd; and the sibling objects in the same drill).
    let want = [item.target_lines, item.target]
    for alt in a:cheats
      if alt ==# item.expected_motion | continue | endif
      enew!
      ...
      call Assert([getline(1, '$'), [line('.'), col('.')]] !=# want,
        \ a:id . '/' . item.expected_motion . ': ' . alt
        \ . ' must NOT reproduce the target (cheat gate)')
      bwipeout!
    endfor
...
function! s:test_delete_inside_quotes() abort
  call s:assert_inner_object_drill('delete_inside_quotes',
    \ ['di"', "di'", 'di`'], ['diw', 'daw', 'dd', 'di"', "di'", 'di`'])
endfunction

function! s:test_delete_inside_angle_vs_tag() abort
  call s:assert_inner_object_drill('delete_inside_angle_vs_tag',
