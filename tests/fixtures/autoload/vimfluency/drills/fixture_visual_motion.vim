" Fixture visual_motion drill. Generated items come from
" g:vf_fixture_visual_items in order; index lives in
" g:vf_fixture_visual_idx. Wraps when the queue is exhausted, same
" pattern as fixture_motion. Lives only in tests/fixtures/ so it never
" appears in a real :VfList.

function! vimfluency#drills#fixture_visual_motion#meta() abort
  return {'id': 'fixture_visual_motion', 'name': 'fixture visual_motion',
    \ 'aim': 60, 'allowed_keys': 'vhjkl', 'kind': 'visual_motion'}
endfunction

function! vimfluency#drills#fixture_visual_motion#generate() abort
