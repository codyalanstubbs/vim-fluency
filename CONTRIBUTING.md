...
- **Preview check** (maintainers): the per-drill preview GIFs are
  generated by auto-playing `:VfDemo`, so a new drill's kind must be
  auto-playable. `preview/verify-demo.sh <slug>` asserts it credits
  (render-free); `make -C preview preview DRILL=<slug>` regenerates its
  GIF.
