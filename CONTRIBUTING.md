...
- **Preview check** (maintainers, in the website repo): the per-drill
  preview GIFs are generated by auto-playing `:VfDemo`, so a new drill's
  kind must be auto-playable. `.vimfluency.com/demo/verify-demo.sh <slug>`
  asserts it credits (render-free); `make -C .vimfluency.com/demo preview
  DRILL=<slug>` regenerates its GIF.
